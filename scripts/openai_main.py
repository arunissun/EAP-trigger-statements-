"""
Trigger Statement Extraction Pipeline - Azure OpenAI Entry Point

A cost-optimized pipeline to extract "Trigger Statements" from PDF reports
using Azure OpenAI GPT-3.5 Turbo.
...
5. LLM Extraction - Extract triggers via Azure OpenAI API

Usage:
    uv run python -m scripts.openai_main
    OR
    python scripts/openai_main.py
"""
...
# Import shared modules (no modifications to existing code)
from scripts.config import DOCUMENTS_FOLDER, OUTPUT_FOLDER
...
# Import Azure OpenAI specific modules
from scripts.openai_config import API_DELAY_SECONDS
from scripts.openai_llm_extractor import extract_triggers_with_openai


# =============================================================================
# Document Metadata Functions (Copied from main.py to avoid modifications)
# =============================================================================
...
def prepare_pdf_payload(pdf_path: Path, document_metadata: dict = None) -> tuple[str, dict]:
    ...
    result = {
        ...
        "model": "gpt-35-turbo",  # Mark which model was used
        "status": "success",
        "pages_analyzed": 0,
        "pages_selected": 0,
...
def process_single_pdf(pdf_path: Path, document_metadata: dict = None) -> dict:
    """
    Process a single PDF file and extract trigger statements using Azure OpenAI.
    """
    ...
    # Step 5: Extract triggers via Azure OpenAI
    print("\n[Step 5] Extracting triggers via Azure OpenAI...")
    llm_result = extract_triggers_with_openai(payload)
...
def main():
    ...
    Processes PDFs using Azure OpenAI GPT-3.5 Turbo.
    """
    print("\n" + "=" * 70)
    print("TRIGGER STATEMENT EXTRACTION PIPELINE (Azure OpenAI GPT-3.5 Turbo)")
    print("=" * 70)
    ...
    # Process only first 2 PDFs for testing (adjust as needed)
    ...
    for pdf_path in test_files:
        ...
        output_file = OUTPUT_FOLDER / "extracted_triggers_openai.json"
